...
public class ClubsTests extends TestBase {
    ...
    public void getClubsReturns200AndValidStructure() {
        ClubsListResponseModel response = Allure.step(
                "Отправка GET /clubs/",
                () -> api.clubs.getClubs()
        );

        Allure.step("Проверка, что ответ не null", () ->
                assertThat(response).isNotNull());

        Allure.step("Проверка поля count", () ->
                assertThat(response.count()).isGreaterThanOrEqualTo(0));

        Allure.step("Проверка поля results", () ->
                assertThat(response.results()).isNotNull());

        Allure.step("Проверка пагинации (results.size() <= count)", () ->
    ...
    public void getClubsCountMatchesResultsSize() {
        ...
        Allure.step("Проверка, что размер results не превышает count", () ->
    ...
    public void getClubsEachClubHasRequiredFields() {
        ...
        if (response.results().isEmpty()) {
            Allure.step("⚠️ Нет клубов для проверки полей");
            return;
        }

        Allure.step("Проверка обязательных полей у " + response.results().size() + " клубов", () -> {
    ...
    public void getClubsPaginationFieldsPresent() {
        ...
        Allure.step("Проверка поля next (если есть следующая страница)", () -> {
